...
def filter_metadata_info(raw_metadata_df: pd.DataFrame, date_: str) -> pd.DataFrame:
    ...
    """
    logger.info(
        "Filtering metadata info", extra={"rows": int(raw_metadata_df.shape[0])}
    )
    ...
    logger.info("Filtered metadata info", extra={"rows": int(filtered_df.shape[0])})
    return filtered_df
...
def transform_metadata_info(raw_metadata_df: pd.DataFrame, date_: str) -> pd.DataFrame:
    ...
    """
    logger.info(
        "Transforming metadata info", extra={"rows": int(raw_metadata_df.shape[0])}
    )
    ...
    new_metadata_df["FechaCancelacion"] = new_metadata_df[
        "FechaCancelacion"
    ].dt.strftime("%d/%m/%Y")
    ...
    new_metadata_df["FechaEmision"] = new_metadata_df["FechaEmision"].dt.strftime(
        "%d/%m/%Y"
    )
    ...
    logger.debug(
        "Transformed metadata info", extra={"rows": int(new_metadata_df.shape[0])}
    )
...
def get_metadata_normalized_column_names(raw_metadata_df: pd.DataFrame) -> list[str]:
    ...
    if missing_columns:
        logger.error(
            "Missing required metadata columns", extra={"missing": missing_columns}
        )
        raise ValueError(
...
def get_edicom_normalized_column_names(raw_edicom_df: pd.DataFrame) -> list[str]:
    ...
    last_clave = next(
        ...
        if ("Unnamed" not in col and col.startswith("CLAVEPRODSERVCONCEPTO"))
    )
    ...
    if len(normalized_edicom_column_names) != len(raw_edicom_column_names):
        logger.error(
            "Mismatch edicom columns length",
            extra={
                "expected": len(raw_edicom_column_names),
                "actual": len(normalized_edicom_column_names),
            },
        )
        raise ValueError(
    ...
    if len(patterns_raw_edicom_column_names) != len(
        ...
        logger.error(
            "Mismatch patterns length",
            extra={
                "patterns_raw": len(patterns_raw_edicom_column_names),
                "patterns_normalized": len(patterns_normalized_edicom_column_names),
            },
        )
        raise ValueError(
    ...
    logger.debug(
        "Selected edicom columns for transformation",
        extra={"selected": selected_raw_edicom_cols},
    )
...
def transform_edicom_info(raw_edicom_df: pd.DataFrame) -> pd.DataFrame:
    ...
    new_edicom_df["FECHADOCUMENTO"] = pd.to_datetime(
        new_edicom_df["FECHADOCUMENTO"], dayfirst=True, errors="coerce"
    )

    new_edicom_df["FECHADOCUMENTO"] = new_edicom_df["FECHADOCUMENTO"].dt.strftime(
        "%d/%m/%Y"
    )

    new_edicom_df["FECHAREAL"] = pd.to_datetime(
        new_edicom_df["FECHAREAL"], dayfirst=True, errors="coerce"
    )

    new_edicom_df["FECHAREAL"] = new_edicom_df["FECHAREAL"].dt.strftime(
        "%d/%m/%Y %H:%M:%S"
    )
    ...
    logger.debug(
        "Transformed edicom dataframe", extra={"rows": int(new_edicom_df.shape[0])}
    )
...
def transform_cfdi_info(raw_cfdi_df: pd.DataFrame, date_: str) -> pd.DataFrame:
    ...
    transformer_cfdi_df = transformer_cfdi_df[
        transformer_cfdi_df["TIPO_COMPROBANTE"] == "I"
    ]
    year, month = map(int, date_.split("_"))
    transformer_cfdi_df["FECHA"] = pd.to_datetime(transformer_cfdi_df["FECHA"])
    ...
    transformer_cfdi_df["FECHA_PERIODO"] = transformer_cfdi_df["FECHA"].dt.month.map(
        MONTH_MAP
    )
    transformer_cfdi_df = transformer_cfdi_df.rename(
        columns={"CONCEPTO_IVA": "% DE IVA POR CONCEPTO"}
    )
